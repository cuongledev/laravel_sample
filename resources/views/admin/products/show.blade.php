                ...
                <div class="col-md-12">
                    <form action="{{ route('admin.product.update',['id' => $product->id]) }}" method="post"
                        ...
                        <div class="form-group {{ $errors->has('sale_price') ? 'has-error' : '' }}">
                            <label for="name">Giá thị trường:</label>
                            <input type="number" min="1" class="form-control" id="sale_price" name="sale_price"
                        ...
                        <div class="form-group {{ $errors->has('original_price') ? 'has-error' : '' }}">
                            ...
                            <input type="number" min="1" class="form-control" id="original_price" name="original_price"
                        ...
                        <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                            ...
                            <div>
                                @if(!empty($product->image) && file_exists(public_path(getThumbnail('uploads/'.$product->image))))
                                    <img src="{{ asset(getThumbnail('uploads/'.$product->image)) }}" width="100px"
                                         height="100px"/>
                                @else
                                    <img src="{{ asset('images/no_image_200x150.jpg') }}" width="100px" height="100px"/>
                                @endif
                        ...
                        <div class="form-group {{ $errors->has('images.*') ? 'has-error' : '' }}">
                            <label for="name">Thư viện hình sản phẩm:</label>
                            <div>
                                @forelse($product->attachments as $file)
                                    @if(file_exists(public_path(getThumbnail('uploads/'.$file->path))))
                                        <img src="{{ asset(getThumbnail('uploads/'.$file->path)) }}" width="100px"
                                             height="100px"/>
                                    @else
                                        <img src="{{ asset('images/no_image_200x150.jpg') }}" width="100px"
                                             height="100px"/>
                                    @endif
    ...
    <script type="text/x-template" id="cn-attributes-template">
        ...
        <table class="table table-striped">
            ...
            <tr v-for="(item,key) in attributes">
                <td><input type="text" v-model="item.name" v-bind:name="'attributes['+key+'][name]'"
                           placeholder="Thuộc tính" class="form-control"/></td>
                <td><input type="text" v-model="item.value" v-bind:name="'attributes['+key+'][value]'"
                           placeholder="Giá trị" class="form-control"/></td>
                <td>
                    <button type="button" v-if="key != 0" v-on:click="delAttributes(item)" class="btn btn-danger"><i
                                class="glyphicon glyphicon-remove"></i></button>
                    <button type="button" v-if="key == attributes.length - 1" v-on:click="addAttributes"
